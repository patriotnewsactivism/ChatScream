...
const App = () => {
  ...
  const { user, userProfile, logout, refreshProfile, sessionToken } = useAuth();
  const mobileLayout = useMobileLayout();
  const { mobilePanel, setMobilePanel, isLandscape, isCompactLandscape, mobileTip, setMobileTip } =
    mobileLayout;
  const {
    streamTopic,
    setStreamTopic,
  ...
  useEffect(() => {
    if (aiError && window.innerWidth < 1024) {
  ...
  const initCam = async (desiredFacingMode: 'user' | 'environment' = cameraFacingMode) => {
    ...
    try {
      ...
      if (
        ...
        err.message?.includes('no tracks')
      ) {
        setCameraStream(null);
        setIsMicMuted(true);
        setIsCamMuted(true);
  ...
  const ensureAudioContext = () => {
    ...
    if (audioPlayerRef.current) {
      try {
        ...
        console.warn('Could not connect music source', e);
      }
    }
  ...
  useEffect(() => {
    ...
    if (
      ...
      !videoGainRef.current ||
      !audioDestRef.current ||
      !videoAnalyserRef.current
    )
      return;
  ...
  useEffect(() => {
    if (audioContextRef.current && micGainRef.current && cameraStream) {
      ...
      try {
        ...
        console.warn('Could not reconnect mic', e);
      }
    }
  }, [cameraStream]);
  ...
  const toggleRecording = () => {
    ...
    if (appState.isRecording) {
      ...
      setAppState((prev) => ({
        ...prev,
        isRecording: false,
        streamDuration: prev.isStreaming ? prev.streamDuration : 0,
      }));
    } else {
      try {
        ...
        if (err.name !== 'NotAllowedError') {
          alert(
            err.message.includes('permission')
              ? 'Screen sharing blocked by permission policy.'
  ...
  const copyToClipboard = async (text: string) => {
    try {
      ...
      if (window.innerWidth < 1024) {
        mobileLayout.setMobileTip('Copied to clipboard.');
      }
    } catch (err) {
  ...
  const handleLogout = async () => {
    ...
    try {
      await logout();
      navigate('/');
      mobileLayout.setMobileTip('Signed out.');
    } catch (error) {
      console.error('Logout failed:', error);
      mobileLayout.setMobileTip('Sign out failed. Please try again.');
    }
  };
  ...
  const activeVideoUrl = activeVideoId
    ? mediaAssets.find((a) => a.id === activeVideoId)?.url || null
    : null;
  ...
  const renderGraphicsPanel = () => <BrandingPanel settings={branding} onChange={setBranding} />;
      ...
      <div className="flex-1 overflow-y-auto pb-safe">
        <DestinationManager
          ...
          onToggleDestination={(id) =>
            setDestinations(
              destinations.map((d) => (d.id === id ? { ...d, isEnabled: !d.isEnabled } : d)),
            )
          }
          ...
          userId={user?.uid}
          connectedPlatforms={{
            youtube: userProfile?.connectedPlatforms?.youtube
              ? { channelName: userProfile.connectedPlatforms.youtube.channelName }
              : undefined,
            facebook: userProfile?.connectedPlatforms?.facebook
              ? { pageName: userProfile.connectedPlatforms.facebook.pageName }
              : undefined,
            twitch: userProfile?.connectedPlatforms?.twitch
              ? { channelName: userProfile.connectedPlatforms.twitch.channelName }
              : undefined,
          }}
          onUpgradeClick={() => {
            setMobilePanel('none');
  ...
  return (
    <div
      className="h-[100dvh] w-full bg-dark-900 text-gray-100 flex flex-col overflow-hidden"
    ...
    >
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-black relative">
          <div
            className={`flex-1 flex items-center justify-center relative bg-[#0a0a0a] overflow-hidden ${isCompactLandscape ? 'p-0' : 'p-3 md:p-8'}`}
          >
            {/* Video Container */}
            <div
              className={`transition-all duration-300 relative shadow-2xl border-gray-800 overflow-hidden bg-black
            ...
            >
              ...
              />
              {/* Chat Stream Overlay */}
              <ChatStreamOverlay messages={broadcastMessages} style="default" position="bottom" />
            </div>
              ...
              <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-6">
                <div className="bg-dark-800 p-6 rounded-xl border border-red-500/50 max-w-sm w-full text-center shadow-2xl animate-fade-in">
                  ...
                  <div className="flex flex-col gap-3">
                    <button
                      onClick={() => initCam()}
                      className="w-full bg-brand-600 text-white px-4 py-3 rounded-lg font-bold"
                    >
                      Try Again
                    </button>
                    <button
                      onClick={handleContinueWithoutCam}
                      className="w-full bg-gray-700 text-gray-200 px-4 py-3 rounded-lg font-semibold"
                    >
          ...
          {isLandscape && mobilePanel !== 'none' && (
            <div className="fixed inset-y-16 right-0 w-full sm:w-[50%] md:w-[40%] bg-dark-900/95 backdrop-blur border-l border-gray-700 z-40 animate-slide-up flex flex-col shadow-2xl">
              <div className="flex items-center justify-between p-3 border-b border-gray-700">
                ...
                <button onClick={() => setMobilePanel('none')}>
                  <X size={18} />
                </button>
              </div>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-sm font-bold uppercase text-gray-200 flex items-center gap-2 mt-2">
                    ...
                    {mobilePanel === 'graphics' && (
                      <>
                        <Palette size={16} /> Stream Graphics
                      </>
                    )}
          ...
          <div className="bg-dark-800 border-t border-gray-700 z-30 shrink-0 flex flex-col pb-safe shadow-2xl">
            ...
            <div className="lg:hidden flex border-t border-gray-700 bg-dark-900 safe-area-bottom">
              ...
              >
                <Palette size={20} />{' '}
                <span className="text-[10px] sm:text-xs font-bold">STYLE</span>
              </button>
              <button
                onClick={() =>
                  setMobilePanel(mobilePanel === 'destinations' ? 'none' : 'destinations')
                }
