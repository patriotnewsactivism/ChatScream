...
### 1. `services/sanitize.ts` — **Critical priority**
...
- `sanitizeUrl`: blocks `javascript:`, `data:`, `vbscript:` — allows `http/https`, relative paths, `mailto`, `tel`; prepends `https://` to bare domains
...
### 2. `services/stripe.ts` — **Critical priority**

Core business logic that controls feature access, destination limits, and billing. Pure
functions with no network calls — entirely unit-testable.
...
- `getScreamTier`: below $5 returns null, $5–$9.99 → standard, $10–$49.99 → loud, $50+ → maximum
...
### 3. `services/streamEnforcement.ts` — **High priority**
...
### 4. `services/claudeService.ts` — **High priority**
...
**What to test:**
- `fallbackViralPackage`: generates correct hashtag from topic slug, handles special characters, enforces title length ≤60, description ≤220, max 12 hashtags and 15 tags
...
### 5. `services/bitrateAdaptation.ts` — **Medium priority**
...
- `predictNextBitrate`: requires ≥5 history samples, linear trend extrapolation
- `setManualBitrate`: disables adaptation, updates profile, fires callback
- `updateConfig`: merges partial config correctly

---

### 6. `services/streamHealthMonitor.ts` — **Medium priority**
...
**What to test:**
- `analyzeHealth`: bitrate below threshold → warning; bitrate < 50% of target → critical; FPS, dropped frames, RTT, packet loss, CPU, memory, encoder thresholds all fire correct severity
...
### 7. `components/ErrorBoundary.tsx` and `ChunkErrorBoundary.tsx` — **Medium priority**
...
### 8. `hooks/useRealtimeChat.ts` — **Medium priority**

Manages live chat state — connection status, message list, error handling, send throttling.
...
### 9. `pages/AuthPage.tsx` and `pages/OAuthCallback.tsx` — **Medium priority**
