    ...
    <div className="bg-dark-800/90 backdrop-blur-md border border-gray-700 rounded-lg px-3 py-2 space-y-1.5">
      {videoName && (
        <div className="flex items-center justify-between">
          <span className="text-xs font-medium text-gray-300 truncate max-w-[60%]">🎬 {videoName}</span>
          <button onClick={onStop} className="text-[10px] text-red-400 hover:text-red-300 font-bold uppercase">
            Stop & Remove
          </button>
        </div>
      )}
      ...
      <div className="flex items-center justify-center gap-3">
        <button onClick={() => skip(-10)} className="p-1.5 rounded hover:bg-gray-700 text-gray-400 hover:text-white" title="Back 10s">
          <SkipBack size={14} />
        </button>
        <button onClick={togglePlay} className="p-2 rounded-full bg-brand-500 hover:bg-brand-600 text-white" title={paused ? 'Play' : 'Pause'}>
          {paused ? <Play size={16} fill="currentColor" /> : <Pause size={16} />}
        </button>
        <button onClick={() => skip(10)} className="p-1.5 rounded hover:bg-gray-700 text-gray-400 hover:text-white" title="Forward 10s">
          <SkipForward size={14} />
        </button>

        <div className="ml-3 flex items-center gap-1.5">
          <button onClick={() => onVolumeChange(volume > 0 ? 0 : 0.8)} className="p-1 text-gray-400 hover:text-white">
